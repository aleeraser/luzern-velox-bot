...
async def cmd_current_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
async def cmd_manual_update(_update: Update, context: ContextTypes.DEFAULT_TYPE):
    return await check_for_updates(context.application, forced_update=True)


# command to handle /notify_no_updates
async def cmd_set_notify_no_updates(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(chat_id=update.message.chat_id, text=f"{msg}")
...
def bot_start():
    ...
    try:
        ...
        print(
            "Error: no valid config.json found. A template has been created,"
...
parser.add_argument(
    "-t", "--telegram-bot", action="store_true", help="Start the Telegram bot"
)
parser.add_argument(
    "-s",
    "--save-list",
...
parser.add_argument(
    "-p", "--print-list", action="store_true", help="[CLI] Print the current list"
)
